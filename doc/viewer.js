document.addEventListener('DOMContentLoaded', function () {
  ...
  var sourceText = mdSourceEl.textContent.trim();
  ...
  if (sourceText.endsWith('.md')) {
    fetch(sourceText)
      ...
      .then(renderMarkdown)
      .catch(function(e) {
        el.innerHTML = '<p style="color: red;">Error fetching content: ' + e.message + '</p>';
      });
  } else {
    try {
      renderMarkdown(JSON.parse(sourceText));
    } catch (e) {
